...
import { resolveTeam, getAllTeams } from "./teams.js";
...
const persistDir = join(homedir(), ".my-pixel-office");
...
const topicCachePath = join(homedir(), ".my-pixel-office", "session-topics.json");
...
function saveTopicCache(): void {
  try {
    mkdirSync(join(homedir(), ".my-pixel-office"), { recursive: true });
...
function sendInitialData(ws: WebSocket): void {
  ...
  // Send teams list first
  ws.send(JSON.stringify({ type: "teamsUpdated", teams: getAllTeams() }));

  // Send existing agents with persisted seat metadata
  const agentList = Array.from(agents.values());
  const agentIds = agentList.map((a) => a.id);
  const folderNames: Record<number, string> = {};
  const agentMeta: Record<number, { palette?: number; hueShift?: number; seatId?: string; team?: unknown }> = {};
...
watcher.on("fileAdded", (file: WatchedFile) => {
  ...
  const team = resolveTeam(file.projectName, dirname(file.path));
  ...
  broadcast({ type: "agentCreated", id: agent.id, folderName: agent.projectName, sessionTopic, team });
...
const onServerError = (err: NodeJS.ErrnoException) => {
  if (err.code === 'EADDRINUSE') {
    console.error(`[Server] 포트 ${PORT}가 이미 사용 중입니다. 기존 프로세스를 종료 후 재시작하세요.`)
  } else {
    console.error('[Server] 치명적 오류:', err.message)
  }
  process.exit(1)
}
