...
function formatToolStatus(toolName: string, input: Record<string, unknown>): string {
  ...
  switch (toolName) {
    case "Read":
      return `${base(input.file_path)} \uc77d\ub294 \uc911`;
    case "Edit":
      return `${base(input.file_path)} \ud3b8\uc9d1 \uc911`;
    case "Write":
      return `${base(input.file_path)} \uc791\uc131 \uc911`;
    case "Bash": {
      const cmd = (input.command as string) || "";
      return `\uba85\ub839 \uc2e4\ud589: ${cmd.length > BASH_COMMAND_DISPLAY_MAX_LENGTH ? cmd.slice(0, BASH_COMMAND_DISPLAY_MAX_LENGTH) + "\u2026" : cmd}`;
    }
    case "Glob":
      return "\ud30c\uc77c \uac80\uc0c9 \uc911";
    case "Grep":
    ...
    case "Task": {
      ...
      return desc
        ? `\uc11c\ube0c\ud0dc\uc2a4\ud06c: ${desc.length > TASK_DESCRIPTION_DISPLAY_MAX_LENGTH ? desc.slice(0, TASK_DESCRIPTION_DISPLAY_MAX_LENGTH) + "\u2026" : desc}`
        : "\uc11c\ube0c\ud0dc\uc2a4\ud06c \uc2e4\ud589 \uc911";
    }
    case "Agent":
      return "\uc11c\ube0c\uc5d0\uc774\uc804\ud2b8 \uc2e4\ud589 \uc911";
