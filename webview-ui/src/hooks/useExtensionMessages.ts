...
function toolNameToZoneClient(toolName: string): string {
...
export function useExtensionMessages(
  ...
  onTeamsUpdated?: (teams: Array<{ id: string; name: string; color: string }>) => void,
  isEditDirty?: () => boolean,
  ...
  useEffect(() => {
    ...
    const handler = (e: MessageEvent) => {
      ...
      if (msg.type === 'layoutLoaded') {
      ...
      } else if (msg.type === 'teamsUpdated') {
        const teams = msg.teams as Array<{ id: string; name: string; color: string }>
        console.log('[Teams] 업데이트:', teams.map(t => t.name).join(', '))
        onTeamsUpdated?.(teams)
      } else if (msg.type === 'agentCreated') {
        const id = msg.id as number
        const folderName = msg.folderName as string | undefined
        const sessionTopic = msg.sessionTopic as string | undefined
        const team = msg.team as { id: string; name: string; color: string; zone: string } | undefined
        ...
        if (team) {
          ...
          if (ch) (ch as typeof ch & { team?: unknown }).team = team
        }
        saveAgentSeats(os)
        ...
        // Move agent to the appropriate activity zone
        const zoneId = toolNameToZoneClient(toolName ?? '')
        os.setAgentActivityZone(id, zoneId)
        ...
        os.clearPermissionBubble(id)
        os.setAgentActivityZone(id, 'idle')
        ...
        if (status === 'waiting') {
          ...
          os.setAgentActivityZone(id, 'comms')
          playDoneSound()
        }
