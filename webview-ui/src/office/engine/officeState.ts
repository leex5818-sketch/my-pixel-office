...
// Chair UIDs generated by isoLayout: uid counter starts at 1, each zone produces
// desk-N, chair-(N+1), desk-(N+2), chair-(N+3), partition, plant, whiteboard/cooler
// newsletter: chair-2, chair-4 | research: chair-9, chair-11 | data: chair-16, chair-18
// design: chair-23, chair-25 | ops: chair-30, chair-32 | shared: chair-37, chair-39
...
// Candidate tiles in the break room — shared zone interior (col 21, row 10, w=8, h=7)
// Interior floor: cols 22-27, rows 11-15 (walls at zone border are excluded)
...
export class OfficeState {
  ...
  /** Move an agent to the zone matching the given activity zone ID */
  setAgentActivityZone(agentId: number, zoneId: string): void {
    const ch = this.characters.get(agentId)
    if (!ch || ch.isSubagent) return

    // Already in this zone?
    const currentTeam = (ch as typeof ch & { team?: { id: string } }).team
    if (currentTeam?.id === zoneId) return

    const layout = this.getLayout()
    const targetZone = layout.zones?.find(z => z.teamId === zoneId)
    if (!targetZone) return

    // Find a free seat in the target zone
    const freeSeatUid = targetZone.preferredSeatUids?.find(uid => {
      const seat = this.seats.get(uid)
      return seat && !seat.assigned
    })

    if (!freeSeatUid) return // zone full, stay put
    ...
    // Claim new seat
    const freeSeat = this.seats.get(freeSeatUid)!
    freeSeat.assigned = true
    ch.seatId = freeSeatUid

    // Update team display
    ;(ch as typeof ch & { team?: { id: string; name: string; color: string; zone: string } }).team = {
      id: zoneId,
      name: targetZone.teamId,
      color: targetZone.color ?? '#888888',
      zone: zoneId,
    }
